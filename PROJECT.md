...
## 十一、待办池（已评估未立项，捡起前先读一遍）

### 聊天记录记忆（QQ/微信检索问答，2026-09-13 评估，老大拍板缓一缓）

**需求**：用户自然语言问自己的聊天记录（"他上次发的地址"、"总结这周群聊"、年度报告）。真需求，MemoTrace 官网百万级用户已验证。

**立项形态（已定，三件套风险切割）**：
1. **主程序 · 聊天记忆**（默认功能，零风险）：导入向导（支持 MemoTrace 导出的 CSV/HTML 格式）→ 本地存库 → AI 对话式检索/总结/年度报告（render_html 出年报比 MemoTrace 漂亮）
2. **独立插件仓库 · 聊天记录增强**（用户自愿下载）：微信/QQ 自动提取（读进程内存抓密钥 → 解密 SQLCipher DB），产出喂给主程序同一个聊天记忆库。**代码绝不进主程序**——杀软扫二进制，解密代码打进主 exe = 默认状态就报毒（设置开关切不掉这个）
3. **设置入口**：「聊天记录增强（实验性）」开关 = 引导下载启用插件

**技术情报（2026-09 摸底）**：
- 微信 4.0 加密大改：SQLCipher v4 + 动态密钥（运行时生成），静态查找成功率仅 15%；PyWxDump 4.0 用运行时捕获（Frida 插桩思路）达 98%/30 秒，但 **PyWxDump 原仓库已删库**（社区镜像续命）——插件版需持续跟随微信版本，维护是无底洞
- MemoTrace（LC044/WeChatMsg）已适配微信 4.0，导出 HTML/Word/CSV/TXT，自述"长期维护模式"——是我们的让位空间+导入格式来源
- QQ NT 架构解析生态比微信弱，优先级放低
- ⚠️ **启动前必查**：WeChatMsg/MemoTrace 的开源许可证（若 GPL/AGPL 传染，只能支持其导出文件格式，不能借代码）

**风险备忘（为什么 B 不进主程序）**：杀软误报（读微信内存必被标记，MemoTrace 都在教用户忽略杀软提示）/ 微信大版本洗牌维护无底洞 / 腾讯法务对实名商业产品 vs 开源个人项目的区别对待。

**重启条件**：主程序聊天记忆可随时开工（纯增量无风险）；插件版等微信 4.x 解析生态稳定或找到可持续维护的密钥捕获方案再评估。
